...
export const Movie: React.FC<MoviePropsType> = (props) => {
    const {poster, title, posterWidth, height, width, elipsis} = props;

    const src = `https://image.tmdb.org/t/p/w${posterWidth}/${poster}`;

    return (
                <MovieSC src={src} height={height} width={width} poster={poster}>
                    <img
                    ...
                    <TitleSC width={width} elipsis={elipsis}>{title && title.length > 48 ? `${title.slice(0,48)}..` : title}</TitleSC>
                </MovieSC>
    )
};